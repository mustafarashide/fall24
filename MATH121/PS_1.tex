	...
	\begin{questions}
		...
		\question Without a calculation or computer, match the functions (a)-(f) with their cross-sections with $x$ fixed in the Figure 3. Explain your reasoning
			...
			\begin{parts}
				...
				\part We are looking for cross-sections showing an increasing value for z as y increases for any fixed value of x. This is the case in (III) so these must be the cross-sections for the function $z = e^{-x+y}$.
				\part We are looking for cross-sections showing a decreasing value for z as y increases for any fixed value of x. This is the case in (VI) so these must be the cross-sections for the function $z = e^{x-y}$.
				\part (V) shows periodic behavior so it has to be the graph showing cross-sections of the function $z=sin(xy)$.
				\part (IV) shows contour lines where $z$ is not changed by y. The z-intercept is always a square number so these are the cross-sections of the function $z=x^2$.
			\end{parts}
		\end{solution}
	\end{questions}
\end{document}
